...
Button(f0, text="CONTROL\nPANEL", width=w, height=h, command=lambda: raise_frame(f1), font=VerdanaL).\
    grid(row=3, column=0)
Button(f0, text="BLACK\nBOX", width=w, height=h, command=lambda: raise_frame(f2), font=VerdanaL).\
    grid(row=3, column=1)
Button(f0, text="TEST", width=w, height=h, command=lambda: raise_frame(f3), font=VerdanaL).grid(row=3, column=2)
Button(f0, text="QUIT", width=w, height=h, command=close, font=VerdanaL).grid(row=3, column=3)
image1 = Image.open(NRM_1)
image_ = ImageTk.PhotoImage(image1)
Label(f0, image=image_).grid(row=0, column=0, columnspan=4)
